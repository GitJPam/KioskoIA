            ...
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-end">
              ...
              <div className="flex flex-wrap gap-2">
                {(["day", "week", "month", "year"] as const).map((opt) => (
                  <Button
                    key={opt}
                    variant={periodOption === opt ? "default" : "outline"}
                    size="sm"
                    onClick={() => handleQuickPeriod(opt)}
                    className={periodOption === opt ? "bg-emerald-600 hover:bg-emerald-700" : ""}
                  >
                    {opt === "day" ? "Hoy" : opt === "week" ? "Semana" : opt === "month" ? "Mes" : "Año"}
                  </Button>
                ))}
              </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <div className="flex gap-2 mt-2">
                    {(["quantity", "revenue", "profit"] as const).map((opt) => (
                      <Button
                        key={opt}
                        variant={sortBy === opt ? "default" : "outline"}
                        size="sm"
                        onClick={() => setSortBy(opt)}
                        className={sortBy === opt ? "bg-emerald-600 hover:bg-emerald-700" : ""}
                      >
                        {opt === "quantity" ? "Por Cantidad" : opt === "revenue" ? "Por Ingresos" : "Por Ganancia"}
                      </Button>
                    ))}
                  </div>
                </CardHeader>
                    ...
                    {profitsData?.desglosePorMetodoPago && profitsData.desglosePorMetodoPago.length > 0 && (
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {profitsData.desglosePorMetodoPago.slice(0, 6).map((method) => (
                            <div
                              key={method.metodo}
                              className="p-2 bg-gray-50 rounded text-center"
                            >
                              <p className="text-xs text-gray-500">{getPaymentMethodLabel(method.metodo)}</p>
                              <p className="text-sm font-bold">{formatPrice(method.ingresos)}</p>
                            </div>
                          ))}
                        </div>
                      </div>
